...
# exit when any command fails; requires -b option on sqlcmd -b
set -e

# Preparation: an active Kerberos ticket for Arthur Goldberg (goldba06) must be available to run sqlcmd -b -E
...
if klist 2> /dev/null | grep -q '>>>Expired<<<'
then
   echo "Error: an active Kerberos ticket for goldba06@MSSMCAMPUS.MSSM.EDU is needed."
   exit 1
fi
...
sqlcmd -b -E -S $MSDW2_SERVER -i empty_custom_Leaf_data_n_their_antecedents.sql
...
sqlcmd -b -E -S $MSDW2_SERVER -i alter_ConceptSqlSet.sql
...
sqlcmd -b -E -S $MSDW2_SERVER -i 1_sqlsets.sql
echo "$(date +%T): running '2_demographics.sql'"
sqlcmd -b -E -S $MSDW2_SERVER -i 2_demographics.sql
echo "$(date +%T): running '3_visits.sql'"
sqlcmd -b -E -S $MSDW2_SERVER -i 3_visits.sql
echo "$(date +%T): running '5_vitals.sql'"
sqlcmd -b -E -S $MSDW2_SERVER -i 5_vitals.sql
echo "$(date +%T): running '8_other.sql'"
sqlcmd -b -E -S $MSDW2_SERVER -i 8_other.sql

##### Load the concepts for clinical domains #####

### Load the conditions concepts ###
echo "$(date +%T): running 'leaf_icd10.sql'"
sqlcmd -b -E -S $MSDW2_SERVER -i ../../leaf_launch/conditions/leaf_icd10.sql
...
sqlcmd -b -E -S $MSDW2_SERVER -i ../../leaf_launch/utilities/coming_soon.sql
...
sqlcmd -b -E -S $MSDW2_SERVER -i post_concept_creation_tasks.sql
