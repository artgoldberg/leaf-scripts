...
### Read sharon's procedure mappings into rpt.leaf_procedures.curated_procedure_mappings ###
...
sqlcmd -b -E -S $MSDW2_SERVER -i load_files/init_file_loads.sql
...
for MAPPING_FILE in Usagi_Procedure_1_SN_MAPPED.txt Usagi_SurgicalProcedures_1_SN_MAPPED.txt Usagi_SurgicalProcedures_2_SN_MAPPED.txt; do
    ...
    sqlcmd -b -E -S $MSDW2_SERVER -i load_files/prep_file_load.sql
    BCP rpt.leaf_procedures.temp_curated_procedure_mappings in "$CURATED_PROCEDURE_MAPPINGS_DIR$MAPPING_FILE" -S $MSDW2_SERVER -T \
        -F 2 -m 10 -c -r0x0A -e errors.log
    sqlcmd -b -E -S $MSDW2_SERVER -i load_files/finish_file_load.sql

done

echo "$(date +%Y-%m-%d\ %T): running 'cleanup_file_loads.sql'"
sqlcmd -b -E -S $MSDW2_SERVER -i load_files/cleanup_file_loads.sql

### Finish creating mappings from 'Epic procedure codes' to CPT ###
echo "$(date +%Y-%m-%d\ %T): running 'procedures.sql'"
sqlcmd -b -E -S $MSDW2_SERVER -i procedures.sql -r1 2> 'procedures_errors.log'
